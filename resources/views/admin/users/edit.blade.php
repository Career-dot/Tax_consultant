...
<div class="page-header anim-fade-up">
    <div class="page-info">
        <div style="display: flex; align-items: center; gap: 12px;">
            ...
            <div>
                <h2>Edit User</h2>
                <p>Update user information and service assignments.</p>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="adm-card-body">
                    ...
                    <div style="display: grid; gap: 16px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                            <div>
                                <label style="display: block; font-size: 12px; font-weight: 600; color: var(--ink-secondary); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.5px;">New Password</label>
                ...
                <div class="adm-card-body">
                    ...
                    <div style="display: grid; gap: 10px;">
                        @foreach($services as $service)
                            <label style="display: flex; align-items: center; gap: 12px; padding: 12px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); cursor: pointer; transition: all 0.15s ease; {{ $user->services->contains($service->id) ? 'border-color: var(--primary); background: var(--primary-50);' : '' }}" onmouseover="this.style.borderColor='var(--primary)'" onmouseout="if(!this.classList.contains('selected'))this.style.borderColor='var(--border)'">
                                <input type="checkbox" name="services[]" value="{{ $service->id }}" {{ $user->services->contains($service->id) ? 'checked' : '' }}
