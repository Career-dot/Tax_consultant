...
<style>
.pf-auth-input-icon{position:relative}
.pf-auth-input-icon input{padding-right:2.5rem}
.pf-auth-input-icon .pf-auth-password-toggle{position:absolute;right:-14px;top:50%;transform:translateY(-50%);border:0;background:transparent;padding:.25rem;color:inherit;cursor:pointer}
.pf-auth-input-icon .pf-auth-password-toggle:focus{outline:none}
</style>

<script>
function updateToggleIcon(btn, input) {
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('[data-modal-password-toggle]').forEach(function(btn) {
        const controlId = btn.getAttribute('aria-controls');
        const input = controlId ? document.getElementById(controlId) : null;
        if (!input) return;
        // Ensure initial icon matches input state (password -> fa-eye, text -> fa-eye-slash)
        updateToggleIcon(btn, input);
        // Prevent form submit if button accidentally inside a form
